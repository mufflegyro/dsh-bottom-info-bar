...
- **「信息底栏」设置页（v1.9）**：设置面板新增「信息底栏」页面——**所有字段可独立开关**（含服务商·模型；全部关闭 = 整栏彻底隐藏不留空行），字段分「原生」（DSH 原有统计）与「插件」两组；**每字段可自定义颜色**（预设色板 / 系统取色器 / 色号，浅色与深色主题自动适配可读）；「重置标签」「重置颜色」一键恢复默认；所有设置**持久保存，重启不丢**
