...
| 反馈 | 来源 | 优先级 | 建议方案 |
|---|---|---|---|
| 【性能】记账 journal 永不截断、内存记录数组无限增长；客户端 30s 轮询 + 每次回复后触发，每次全量扫 6+ 遍所有记录，重度使用数月后会持续拖慢宿主 | 安装前检测插件反馈 | 高 | journal 定期压缩（旧明细汇总成摘要后归档）+ 汇总增量缓存（新记录才重算，刷新读缓存）。**审计证实全部属实**：本机 7.8 天已积累 6050 条/2MB，一次花费汇总=8 遍全量扫描，6 个月重度使用约 300MB 磁盘、每次刷新阻塞可达数百毫秒；方案=journal 滚动压缩 + 90 天明细保留窗+按日/按会话汇总 + 增量缓存（详见 docs/PERF-AUDIT-v1.9.md） **✅ v1.9.0 已上线（2026-08-28）** |
| 【功能】信息栏字段自选：现有字段太多，很多信息不想关注，只想显示自己关心的那几个（含原有状态信息全部可选） | 视频评论区用户留言 | 中高 | 提供字段开关，默认全开保持现有体验——**用户拍板（2026-08-28）：做进 DSH 设置面板新板块「信息底栏」（settings.section 插座），不做信息栏弹小窗**：所有标签带勾选开关 + 自定义颜色选择 + 标签/颜色各设独立重置按钮；配置落盘 settings.json 重启不丢，顺带修复现有 density 设置重启即丢的旧问题（详见 docs/FIELDS-AUDIT-v1.9.md） **✅ v1.9.0 已上线（2026-08-28）** |
| 【功能】字段增加配色：不同字段给不同颜色，一眼看到最关注的信息——**用户拍板（2026-08-28）：要支持自定义选色，不只是内置语义色** | 视频评论区用户留言 + 用户拍板 | 中高 | 每字段颜色 = 预设色板快速挑 + 自定义取色器（含明暗双主题自动适配与可读性兜底）；默认全部跟随主题，外观与当前版本一致 **✅ v1.9.0 已上线（2026-08-28）** |
